 ...
 * This is not the only guard — the read-only executor (db-tools.ts here, and
 * mcp/db.ts for the standalone server) also runs every query inside a
 * `BEGIN TRANSACTION READ ONLY` with a statement timeout, so a write that
 * slips past these checks is still rejected by Postgres itself.
 *
 * This module is the single source of truth: mcp/sql-guard.ts re-exports it so
 * the in-app assistant and the standalone MCP server can never drift apart on
 * what SQL is considered safe.
 */
